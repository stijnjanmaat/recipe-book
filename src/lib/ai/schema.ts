 ...
 */
function fixSchemaForStrictMode(schema: Record<string, unknown>): Record<string, unknown> {
  const fixed = { ...schema }

  // If this is an object schema with properties
  if (fixed.type === 'object' && fixed.properties && typeof fixed.properties === 'object') {
    ...
    for (const key of propertyKeys) {
      ...
      if (prop && typeof prop === 'object' && !Array.isArray(prop)) {
        properties[key] = fixSchemaForStrictMode(prop as Record<string, unknown>)
      }
    }
  }

  // If this is an array schema, fix the items schema
  if (fixed.type === 'array' && fixed.items && typeof fixed.items === 'object') {
    fixed.items = fixSchemaForStrictMode(fixed.items as Record<string, unknown>)
  }
